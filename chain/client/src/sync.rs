...
impl EpochSync {
    pub fn new(
        ...
        Self {
            ...
            last_request_time: Utc::now_or_mock(),
...
impl HeaderSync {
    pub fn new(
        ...
        HeaderSync {
            network_adapter,
            history_locator: vec![],
            prev_header_sync: (Utc::now_or_mock(), 0, 0, 0),
            syncing_peer: None,
            stalling_ts: None,
...
impl BlockSync {
    ...
    pub fn block_sync(
        ...
        let request = BlockSyncRequest { height: next_height, hash: next_hash, when: Utc::now_or_mock() };
    ...
    fn block_sync_due(&mut self, chain: &Chain) -> Result<bool, near_chain::Error> {
        match &self.last_request {
            ...
            Some(request) => Ok(chain.head()?.height >= request.height
                ...
                || Utc::now_or_mock() - request.when > Duration::seconds(BLOCK_REQUEST_TIMEOUT)),
        }
    }
}
...
impl PendingRequestStatus {
    fn new(timeout: Duration) -> Self {
        Self { missing_parts: 1, wait_until: Utc::now_or_mock().add(timeout) }
    }
    fn expired(&self) -> bool {
        Utc::now_or_mock() > self.wait_until
    }
}
